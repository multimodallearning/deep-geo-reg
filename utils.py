...
from kaolin.cuda.furthest_point_sampling import furthest_point_sampling
...
class TPS:       
    ...
    def z(x, c, theta):
        ...
        w, a = theta[:-4], theta[-4:]
        b = torch.matmul(U, w)
        return (a[0].unsqueeze(1) + a[1].unsqueeze(1)*x[:, 0] + a[2].unsqueeze(1)*x[:, 1] + a[3].unsqueeze(1)*x[:, 2] + b.t()).t()
...
def thin_plate_dense(x1, y1, shape, lambd=0., unroll_factor=100):
    ...
    tps = TPS()
    theta = tps.fit(kpts_world(x1, (D, H, W)).squeeze(0), (y1.flip(-1) * (torch.Tensor([D, H, W]).to(device) - 1) / 2).squeeze(0), lambd)
    ...
    return (y2.permute(1, 0).view(1, 3, D, H, W)).flip(1) * 2 / (torch.Tensor([W, H, D]).to(device) - 1).view(1, -1, 1, 1, 1)
...
def farthest_point_sampling(kpts, num_points):
    ...
    ind = furthest_point_sampling(kpts_rand, num_points).long()[0]
    if N < num_points:
...
def kpts_to_img(kpts, shape, radius, sigma):
    ...
    grid = (torch.stack(torch.meshgrid(torch.linspace(-radius, radius, radius * 2 + 1),
                                       ...
                                       torch.linspace(-radius, radius, radius * 2 + 1)), dim=-1).to(device) + kpts.round().view(-1, 1, 1, 1, 3)).view(-1, 3).long()
    grid[:, 0].clamp_(0, D - 1)
    grid[:, 1].clamp_(0, H - 1)
    grid[:, 2].clamp_(0, W - 1)
    ...
    grid_disp = (torch.stack(torch.meshgrid(torch.linspace(- radius, radius, radius * 2 + 1),
                                            torch.linspace(- radius, radius, radius * 2 + 1),
                                            torch.linspace(- radius, radius, radius * 2 + 1)), dim=-1).to(device) - disp.view(-1, 1, 1, 1, 3)).view(N, -1, 3)
...
def copdgene_error(path, idx, lms_inh_displacement):
    ...
    dists = torch.sqrt(torch.sum((voxel_spacing.unsqueeze(0) * torch.round(lms_inh_est[:, :3] - lms_exh[:, :3])) ** 2, dim=1))
...
def overlay_segment(img, seg):
    ...
    colors = torch.FloatTensor([0, 0, 0, 199, 67, 66, 225, 140, 154, 78, 129, 170, 45, 170, 170, 240, 110, 38, 111, 163, 91, 235, 175, 86, 202, 255, 52, 162, 0, 183]).view(-1, 3).to(device) / 255.0
